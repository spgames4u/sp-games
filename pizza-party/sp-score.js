...
(function() {
    ...
    const guardKey = '__SP_SCORE_RUNNING_' + (
        ...
        || (() => {
            ...
            return parts[parts.length - 1] || 'pizza-party';
        })()
    );
    if (window[guardKey]) {
    ...
    function handleScore(data) {
        const rawScore = data;
        const iScore = typeof rawScore === 'number' ? rawScore : parseInt(rawScore, 10);
        
        const stack = new Error().stack || '';
        const fromGame = stack.includes('c2runtime');
        
        if (!fromGame) {
            const disp = (typeof iScore === 'number' && !isNaN(iScore)) ? iScore : 0;
    ...
    window.__ctlArcadeSaveScore = function(s) {
        ...
        console.log('%c🍕 Pizza score saved: ' + raw, 'color: #E65100; font-weight: bold');
    };
    ...
    function newCtlArcadeSaveScore(iScore) {
        ...
        if (!fromGame) {
            console.log('%c🍕 Pizza score saved: ' + iScore, 'color: #E65100; font-weight: bold');
            return;
        }
