...
(function() {
    ...
    const guardKey = '__SP_SCORE_RUNNING_' + (
        ...
        || (() => {
            ...
            return parts[parts.length - 1] || 'cricket-fielder-challenge';
        })()
    );
    if (window[guardKey]) {
    ...
    function newCtlArcadeSaveScore(iScore) {
        ...
        const fromGame = stack.includes('CGame') || stack.includes('CEndPanel') || stack.includes('CMain') || stack.includes('jquery') || stack.includes('trigger') || stack.includes('dispatch');
        
        if (!fromGame) {
            const raw = (typeof iScore === 'object' && iScore !== null && 'score' in iScore) ? iScore.score : iScore;
            const disp = (typeof raw === 'number' && !isNaN(raw)) ? raw : 0;
            console.log('%c✅ New high score: ' + disp + '!', 'color: #00c853; font-weight: bold');
            return;
        }
        
        const sanitizedScore = Math.floor(Math.abs(typeof iScore === 'object' && iScore !== null && 'score' in iScore ? iScore.score : iScore)) || 0;
    ...
    window.__ctlArcadeSaveScore = function(s) {
        ...
        console.log('%c✅ New high score: ' + raw + '!', 'color: #00c853; font-weight: bold');
    };
