...
(function() {
    ...
    function getGameSlug() {
        ...
        return parts[parts.length - 1] || 'football-ping-pong';
    }
    ...
    function detectScoreToWin(data) {
        try {
            if (!data?.project) return;
            const str = JSON.stringify(data.project);
            const match = str.match(/\[1,"Score_to_Win",\s*0,\s*(\d+)/);
            if (match) {
                const n = parseInt(match[1], 10);
                if (n > 0 && n <= 999) scoreToWin = n;
            }
        } catch (e) {}
    }
    ...
    async function detectProjectId() {
        try {
            ...
            detectScoreToWin(data);
            log('Score_to_Win:', scoreToWin);
            if (data.project?.[31]) {
    ...
    async function detectScore() {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(data)) {
                ...
                if (k.includes('best') || k.includes('high') || k.includes('score') || k.includes('points')) best = Math.max(best, num);
                if (k.includes('last')) last = Math.max(last, num);
                if (k.includes('goal') || k.includes('counter') || k.includes('host') || k.includes('peer')) maxGoals = Math.max(maxGoals, num);
            }
            let detected = Math.floor(last || best) || null;
            if (maxGoals >= scoreToWin && (detected === null || detected < 1000)) detected = 1000;
    ...
    window.ctlArcadeReportGoal = function(count) {
        const n = Math.floor(Math.abs(count)) || 0;
        if (n >= 0) {
            console.log('%c⚽ Goals: ' + n, 'color: #2196f3; font-weight: bold');
        }
    };

    window.ctlArcadeSaveScore = function(s) {
        const raw = typeof s === 'object' && s !== null && 'score' in s ? s.score : s;
        const sanitized = Math.floor(Math.abs(raw)) || 0;
        if (sanitized >= CONFIG.minScore) {
            log('⚽ Result recorded: ' + sanitized);
            updateScoreHistory(sanitized);
            recordSnapshot(sanitized);
            if (!proofState.hasInput) proofState.hasInput = true;
            sendScore(sanitized);
        }
    };
    ...
    async function init() {
        ...
        if (window.c3_runtimeInterface) {
            try {
                window.c3_runtimeInterface.AddRuntimeComponentMessageHandler('runtime', 'sp-save-score', (data) => {
                    const score = (data && typeof data.score === 'number') ? data.score : 1000;
                    const sanitized = Math.floor(Math.abs(score)) || 0;
                    if (sanitized >= CONFIG.minScore) {
                        log('Score from game:', sanitized);
                        updateScoreHistory(sanitized);
                        recordSnapshot(sanitized);
                        if (!proofState.hasInput) proofState.hasInput = true;
                        sendScore(sanitized);
                ...
                log('sp-save-score handler registered');
            } catch (e) { log('Handler registration failed:', e.message); }
        }
