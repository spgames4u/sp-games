...
(function() {
    ...
    function getGameSlug() {
        ...
        return parts[parts.length - 1] || 'princess-goldblade-and-the-dangerous-waters';
    }
        ...
        try {
            ...
            for (const [key, value] of Object.entries(data)) {
                ...
                if (k.includes('levelstatus') && /_\d{2}$/.test(key)) {
                    ...
                    if (v === 1) levelStatusCount++;
                    continue;
                }
                if (k === 'pgdw1_levelnow' || k === 'levelnow') {
                    const levelNow = Math.floor(parseFloat(value));
                    if (levelNow >= 1 && levelNow <= 9) {
                        best = Math.max(best, (levelNow - 1) * 1000);
                    }
                    continue;
                }
                ...
                let score = 0;
                if ((k.includes('level') || k.includes('stage')) && !k.includes('tutorial')) {
                    const level = Math.floor(num);
                    if (level >= 1 && level <= 8) score = level * 1000;
                }
                if (k.includes('best') || k.includes('high') || k.includes('last') || k.includes('score') || k.includes('points') || k.includes('total') || k.includes('completed')) {
                    score = Math.max(score, Math.floor(num));
                }
                if (k.includes('completed') && num >= 1 && num <= 8) {
                    score = Math.max(score, Math.floor(num) * 1000);
                }
                best = Math.max(best, score);
            }
            if (levelStatusCount > 0 && levelStatusCount <= 8) {
                best = Math.max(best, levelStatusCount * 1000);
            }
            const detected = best || null;
            if (detected) {
    ...
    window.ctlArcadeSaveScore = function(s) {
        ...
        if (!fromGame) {
            console.log('%c✅ New high score: ' + s + '!', 'color: #00c853; font-weight: bold');
            return;
        }
