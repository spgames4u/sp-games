...
(function() {
    ...
    function getGameSlug() {
        ...
        return parts[parts.length - 1] || 'tank-wars';
    }
    ...
    let projectId = null;
    let sessionStartScore = null;
    let hadNullScoreBefore = false;
    let lastSentScore = 0;
    let lastSentTime = 0;
    let isSending = false;
    ...
    async function detectScore() {
        ...
        try {
            ...
            let levelStatusCount = 0;
            let completedCount = 0;
            for (const [key, value] of Object.entries(data)) {
                const k = key.toLowerCase();
                if (k.includes('levelstatus') && /_\d{2}$/.test(key)) {
                    if (parseFloat(value) === 1) levelStatusCount++;
                    continue;
                }
                if (k.includes('levels_stars') || k.includes('levels_score')) {
                    try {
                        const parsed = typeof value === 'string' ? JSON.parse(value) : value;
                        if (Array.isArray(parsed)) {
                            completedCount = Math.max(completedCount, parsed.filter(v => (parseFloat(v) || 0) > 0).length);
                        } else if (typeof parsed === 'object' && parsed !== null) {
                            completedCount = Math.max(completedCount, Object.values(parsed).filter(v => (parseFloat(v) || 0) > 0).length);
                        }
                    } catch (e) {}
                    continue;
                }
                if (k.includes('current_level') && !k.includes('tutorial')) {
                    const level = Math.floor(parseFloat(value));
                    if (level >= 2 && level <= 121) completedCount = Math.max(completedCount, level - 1);
                    continue;
                }
            }
            completedCount = Math.max(completedCount, levelStatusCount);
            const pointsPerLevel = 100;
            const maxLevels = 120;
            const detected = completedCount > 0 ? Math.min(completedCount, maxLevels) * pointsPerLevel : null;
            if (detected) {
    ...
    async function poll() {
        ...
        if (!score) {
            hadNullScoreBefore = true;
            return;
        }

        if (sessionStartScore === null) sessionStartScore = score;

        const isFreshThisSession = hadNullScoreBefore;
        const isHigherThanStart = score > sessionStartScore;
        if ((isFreshThisSession || isHigherThanStart) && score > lastSentScore) {
            await sendScore(score);
        } else if ((isFreshThisSession || isHigherThanStart) && (Date.now() - lastSentTime) >= CONFIG.cooldownMs && score > 0 && score !== lastSentScore) {
            await sendScore(score);
        }
    }
    ...
    async function init() {
        ...
        window.addEventListener('beforeunload', async () => {
            cleanup();
            const score = await detectScore();
            const maySend = score && (hadNullScoreBefore || (sessionStartScore !== null && score > sessionStartScore));
            if (maySend) {
                const now = Date.now();
