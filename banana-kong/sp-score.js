...
(function() {
    ...
    const gameSlugFallback = (() => {
        ...
        return new URLSearchParams(location.search).get('gameSlug') || parts[parts.length - 1] || 'banana-kong';
    })();
    ...
    function processScore(iScore) {
        ...
        if (!fromGame) {
            ...
            console.log('%c✅ Banana Kong: Score submitted successfully', 'color: #ff9800; font-weight: bold', raw);
            return;
        }
    ...
    function reinstallHandler() {
        ...
        try {
            if (window.parent !== window && window.parent.__ctlArcadeSaveScore !== window.__ctlArcadeSaveScore) {
                window.parent.__ctlArcadeSaveScore = window.__ctlArcadeSaveScore;
            }
        } catch (e) {}
    }
